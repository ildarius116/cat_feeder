...
void loop() {
 ...
 // You can implement other code in here or just wait a while 
  ...
  while (nextAction == MENU_ON) {
     ...
     button2.tick();
     button3.tick();
     if (nextAction == ACTION_RIGHT && horflag == 1) {
    Serial.println((String) "MENU: Meals per Day: "+mpd+" ");
      while (nextAction == ACTION_RIGHT && horflag == 1) {
         button1.tick();
         button2.tick();
         button3.tick();
         button4.tick();
         button5.tick();
             ...
             delay(500);
                while (nextAction == ACTION_UP || nextAction == ACTION_DOWN) {
                button1.tick();
                button4.tick();
                button5.tick();
             ...
             delay(500);
              ...
              if (nextAction == ACTION_RIGHT && horflag == 2) {
              Serial.println((String) "MENU: Food per Meal: "+fpm+" ");
              while (nextAction == ACTION_RIGHT && horflag == 2) {
              button1.tick();
              button2.tick();
              button3.tick();
              button4.tick();
              button5.tick();
                ...
                fpms++;
                verflag = 1;
                Serial.println("Food per Meal:   ");
                Serial.println((String) "Confirm Food per Meal: "+fpms+" ?");
                delay(500);
                while (nextAction == ACTION_UP || nextAction == ACTION_DOWN) {
                button1.tick();
                button4.tick();
                ...
                if (nextAction == ACTION_CONFIRM) {
                fpm = fpms;
                Serial.println((String) "Food per Meal: "+fpms+" Confirmed");
                //nextAction = MENU_ON;
                
                }
                if (nextAction == MENU_ON) {
                fpms = fpm; 
                Serial.println((String) "Food per Meal: "+fpms+" Canceled");
                }
                }
             }
             if (nextAction == ACTION_DOWN) {
             ...
             fpms--;
             Serial.println("Meals per Day:   ");
             Serial.println((String) "Meals per Day: "+fpms+" ");
             delay(500);
                while (nextAction == ACTION_UP || nextAction == ACTION_DOWN) {
                button1.tick();
                button4.tick();
