  ...
  if (currentAction == MENU_OFF) {
    ...
    if (buttonAction == ACTION_MENU) {
      ...
      Serial.println("Main MENU Quit");   
       lcd.clear();
       lcd.setCursor(0, 0); // set the cursor to column 15, line 0
       lcd.print("Main MENU Quit");// Print a message of "Temp: "to the LCD.
...
void printMenuStatus() {
  ...
  if (numMenu == 0) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0); // set the cursor to column 15, line 0
    lcd.print("Hyeta pervaya !!!!");// Print a message of "Temp: "to the LCD.
  }
  if (numMenu == 1) {
    Serial.println((String) "Pizda vtoraya !!!!");
    lcd.clear();
    lcd.setCursor(0, 0); // set the cursor to column 15, line 0
    lcd.print("Pizda vtoraya !!!!");// Print a message of "Temp: "to the LCD.
  }
  if (numMenu == 2) {
    Serial.println((String) "Hren tretiy !!!!");
    lcd.clear();
    lcd.setCursor(0, 0); // set the cursor to column 15, line 0
    lcd.print("Hren tretiy !!!!");// Print a message of "Temp: "to the LCD.
  }
  printValueStatus();
}
...
void printValueStatus() {
  ...
  lcd.setCursor(0, 1); // set the cursor to column 15, line 1
  lcd.print((String) "Value - " + dataMenu[numMenu]);// Print a message of "Temp: "to the LCD.
}
